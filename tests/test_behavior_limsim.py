...
from tactics2d.behavior.limsim.evaluation import evaluate_planning_result
from tactics2d.behavior.limsim.frenet_planner import FrenetTrajectoryPlanner, ReferencePath
...
def _build_semantic_map():
    ...
    map_.add_roadline(
        RoadLine(
            id_="solid_left",
            geometry=LineString([(0.0, 0.0), (0.0, 80.0)]),
            subtype="solid",
        )
    )
    map_.add_roadline(
        RoadLine(
            id_="dashed_right",
            geometry=LineString([(2.0, 0.0), (2.0, 80.0)]),
            subtype="dashed",
        )
    )
    lane = map_.lanes["A"]
...
def test_reference_path_converts_between_cartesian_and_frenet():
    ...
    reference_path = ReferencePath.from_agent(state, map_, config)
...
def test_frenet_planner_stops_for_stop_target():
    ...
    map_ = _build_semantic_map()
    state = SceneBuilder(config).build(
        {1: _vehicle(1, 0, 1.0, 5.0, speed=8.0)}, map_, frame=0
    )[1]
...
def test_frenet_planner_scores_obstacle_collision_risk():
    ...
    states = SceneBuilder(config).build(
        {
            ...
            2: _vehicle(2, 0, 1.0, 14.0, speed=0.0),
        },
        map_,
        frame=0,
    )
    ...
    keep_reference = ReferencePath.from_agent(states[1], map_, config)
    ...
    assert candidates
    assert min(candidate.cost for candidate in candidates) < max(candidate.cost for candidate in candidates)
...
def test_frenet_planner_adds_junction_conflict_cost():
    ...
    reference_path = ReferencePath.from_agent(ego, map_, config)
...
def test_frenet_planner_stop_fallback_avoids_collision_candidate():
    ...
    states = SceneBuilder(config).build(
        {
            1: _vehicle(1, 0, 1.0, 5.0, speed=8.0),
            2: _vehicle(2, 0, 1.0, 13.0, speed=0.0),
        },
        map_,
        frame=0,
    )
...
def test_background_roi_vehicle_influences_single_agent_action():
    ...
    participants = {
        1: _vehicle(1, 0, 1.0, 5.0, speed=8.0),
        2: _vehicle(2, 0, 1.0, 22.0, speed=0.0),
    }
...
def test_evaluate_planning_result_reports_actions_and_reference_error():
    ...
    participants = {
        1: _vehicle(1, 0, 1.0, 5.0, speed=5.0),
        2: _vehicle(2, 0, 1.0, 20.0, speed=5.0),
    }
    result = LimSimBehaviorModel(config).plan(participants, map_, frame=0)
...
def test_collision_checker_detects_overlapping_rollouts():
    ...
    map_ = _build_parallel_map()
    participants = {
        1: _vehicle(1, 0, 1.0, 5.0, speed=0.0),
        2: _vehicle(2, 0, 1.0, 5.0, speed=0.0),
    }
    states = SceneBuilder(config).build(participants, map_, frame=0)
    follower = LaneFollower(config)
    trajectories = [
