...
def test_pygame_render_manager_smoke():
    ...
    try:
        camera = BEVCamera(0, map_, perception_range=(10, 10, 10, 10))
        manager.add_sensor(camera, window_size=(16, 16), off_screen=True, main_sensor=True)
        manager.update({}, [], 0)
        manager.render()
...
def test_pygame_render_manager_binds_and_removes_missing_sensor():
    ...
    try:
        camera = BEVCamera(1, map_, perception_range=(10, 10, 10, 10))
        hidden_camera = BEVCamera(2, map_, perception_range=(10, 10, 10, 10))
        vehicle = _vehicle(10, 0, 0, heading=0.2)

        manager.add_sensor(camera, window_size=(12, 12), off_screen=False, main_sensor=True)
        manager.add_sensor(hidden_camera, window_size=(12, 12), off_screen=True)
        manager.bind(1, 10)
        ...
        # Remove the vehicle from participants so the bound sensor is unbound.
        manager.update({}, [], 0)
...
def test_top_down_camera_renders_map_and_participants():
    from tactics2d.display.renderers import PygameRenderer
    from tactics2d.display.sensor import BEVCamera
    ...
    camera = BEVCamera(0, map_, perception_range=(10, 10, 10, 10))
    # scale = max(80/20, 80/20) = 4.0
    renderer = PygameRenderer((80, 80), 4.0, (10, 10, 10, 10), map_.boundary)

    # Static (full-map) render
    geometry_data, _, _ = camera.update(0, participants, [1, 2, 3, 4])
    renderer.render(geometry_data)
    first_observation = renderer.get_observation()
    assert first_observation.shape == (80, 80, 3)

    # Tracking render
    geometry_data, _, _ = camera.update(
        0, participants, [1, 2, 3, 4], position=Point(0, 0), heading=0.0
    )
    renderer.render(geometry_data, Point(0, 0), 0.0)
    bound_observation = renderer.get_observation()
    assert bound_observation.shape == (80, 80, 3)
    # At least two distinct colours should be present (background + elements).
...
def test_single_line_lidar_detects_obstacles_and_renders_points():
    from tactics2d.display.renderers import PygameRenderer
    from tactics2d.display.sensor import SingleLineLidar

    map_ = _build_map()
    lidar = SingleLineLidar(1, map_, perception_range=10.0, freq_scan=10.0, freq_detect=80.0)
    lidar.bind_with(1)
    # scale = max(80/20, 80/20) = 4.0
    renderer = PygameRenderer((80, 80), 4.0, (10, 10, 10, 10), map_.boundary)
    ...
    geometry_data, _, _ = lidar.update(0, participants, [1, 2], position=Point(0, 0), heading=0.0)
    renderer.render(geometry_data, Point(0, 0), 0.0, background_color=(0, 0, 0))

    observation = lidar.get_observation()
    assert observation.shape == (8,)
    assert np.isfinite(observation).any()
    assert renderer.surface.get_size() == (80, 80)
...
def test_single_line_lidar_reports_no_hits_without_obstacles():
    ...
    lidar = SingleLineLidar(2, map_, perception_range=5.0, freq_scan=10.0, freq_detect=40.0)

    lidar.update(0, {}, [], position=Point(0, 0), heading=0.0)
