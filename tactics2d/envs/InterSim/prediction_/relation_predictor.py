...
def predict_reactor_for_onepair(target_reactor_id, all_agent_trajectories, track_type_int, time_offset, raw_data,
    ...
    for i in range(0, objects_id.shape[0]):
        ...
        normalizer = utils.Normalizer(all_agent_trajectories_this_batch[i, last_valid_index, 0],
        ...
        stage_one_label = np.argmin(
            [utils.get_dis(lane, all_agent_trajectories_this_batch[i, -1, :2]).min() for lane in lanes]) if len(
            lanes) > 0 else 0
...
class RelationPredictor:
    ...
    def setting_goal_points(self, current_data):
        ...
        for i, agent_id in enumerate(agent_dic):
            ...
            self.data['predicting']['goal_pts'][agent_id] = self.get_goal(current_data=current_data,
                                                                                      agent_id=agent_id,
