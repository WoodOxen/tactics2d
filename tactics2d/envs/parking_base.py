...
def create_vehicle_box(x,y,heading,VehicleBox):
...
def get_box_coords(x,y,heading,VehicleBox):
    vehicle_box = create_vehicle_box(x,y,heading,VehicleBox)
...
def generate_bay_parking(VehicleBox):
    '''
    ...
    rb, _, _, lb  = get_box_coords(origin[0], origin[1], dest_yaw, VehicleBox)
    min_dest_y = -min(rb[1], lb[1]) + MIN_DIST_TO_OBST
    dest_x = origin[0]
    dest_y = random_gaussian_num(min_dest_y+0.4, 0.2, min_dest_y, min_dest_y+0.8)
    car_rb, car_rf, car_lf, car_lb  = get_box_coords(dest_x, dest_y, dest_yaw, VehicleBox)
    ...
    if random()<0.5: # generate simple obstacle
        ...
        rb, _, _, lb  = get_box_coords(left_car_x, origin[1], left_car_yaw, VehicleBox)
        ...
        obstacle_left = create_vehicle_box( left_car_x, left_car_y, left_car_yaw, VehicleBox)
    ...
    if random()<0.5: # generate simple obstacle
        ...
        rb, _, _, lb  = get_box_coords(right_car_x, origin[1], right_car_yaw, VehicleBox)
        ...
        obstacle_right = create_vehicle_box(right_car_x, right_car_y, right_car_yaw, VehicleBox)
    ...
    if random()<0.2: # in this case only a wall will be generate
        ...
        for _ in range(3):
            ...
            obs_yaw = random()*pi*2
            obs_coords = np.array(get_box_coords(obs_x, obs_y, obs_yaw, VehicleBox))
    ...
    while not start_box_valid:
        ...
        start_box = create_vehicle_box(start_x, start_y, start_yaw, VehicleBox)
        # check collision
        for obst in obstacles:
    ...
    if DEBUG:
        ax.add_patch(plt.Polygon(xy=get_box_coords(start_x, start_y, start_yaw, VehicleBox), color='g'))
    ...
    if generate_success:
        ...
        # print(1)
        return generate_bay_parking(VehicleBox)

def generate_parallel_parking(VehicleBox):
    '''
    ...
    rb, rf, _, _  = get_box_coords(origin[0], origin[1], dest_yaw, VehicleBox)
    ...
    if random()<0.5: # generate simple obstacle
        ...
        rb, rf, _, _  = get_box_coords(left_car_x, origin[1], left_car_yaw, VehicleBox)
    ...
    if random()<0.5: # generate simple obstacle
        ...
        rb, rf, _, _  = get_box_coords(right_car_x, origin[1], right_car_yaw, VehicleBox)
    ...
    if generate_success:
        ...
        return generate_parallel_parking(VehicleBox)
