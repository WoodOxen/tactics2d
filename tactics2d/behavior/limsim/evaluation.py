...
class RollingEvaluation:
    ...
    ``action_switch_count`` counts high-level action changes for agents seen in
    consecutive planning rounds. ``memory_hit_count`` counts predictions that
    exactly reuse the remaining committed trajectory from the previous round.
    ``roi_sizes`` and ``background_sizes`` record the selected inner and outer
    RoI vehicle counts for each round.
    """
...
def evaluate_rolling_result(
    ...
    """Evaluate a rolling LimSim simulation with safety and continuity metrics."""
...
def count_action_switches(results) -> int:
    """Count high-level action changes for agents observed in consecutive planning rounds."""
