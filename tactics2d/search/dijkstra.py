...
class Dijkstra:
    ...
    def plan_graph(
        ...
        """Find the shortest path on a generic directed weighted graph.

        Args:
            start_idx (int): Starting node index.
            target_idx (int): Target node index.
            graph (csr_matrix): Sparse adjacency matrix of the search graph.
            max_iter (int): Maximum number of iterations before giving up.
            callback (Optional[Callable[[Dict], None]]): Optional callback function.

        Returns:
            Tuple[List[int], float]: Path indices and total cost. If no path exists,
            returns ``([], inf)``.
        """
        ...
        while open_set and iteration < max_iter:
            ...
            if current_idx == target_idx:
                path = Dijkstra._reconstruct_index_path(came_from, current_idx)
                if callback is not None:
                    callback(
                        {
                            "iteration": iteration,
