...
class BasePlanner(EnvPlanner):
    ...
    def make_predictions(self, current_state, current_frame_idx, ego_agent_id):
        ...
        if self.predict_env_for_ego_collisions:
            ...
            for each_agent in current_state['agent']:
                ...
                # check distance
                if utils.euclidean_distance(current_state['agent'][ego_agent_id]['pose'][current_frame_idx - 1, :2],
                ...
                if int(current_state['agent'][each_agent]['type']) not in self.vehicle_types:
                    ...
                    each_agent_current_v_per_step = utils.euclidean_distance(
                        each_agent_pose[current_frame_idx - 1, :2],
                        each_agent_pose[current_frame_idx - 6, :2]) / 5
                    each_agent_current_a_per_step = (utils.euclidean_distance(
                        each_agent_pose[current_frame_idx - 1, :2],
                        each_agent_pose[current_frame_idx - 6, :2]) / 5 - utils.euclidean_distance(
                    ...
                    steady_in_past = utils.euclidean_distance(
                        each_agent_pose[current_frame_idx - 1, :2],
                    ...
                    if current_lane in current_state['road'] and 'speed_limit' in current_state['road'][current_lane]:
                        ...
                        my_target_speed = speed_limit if speed_limit is not None else plan_helper.mph_to_meterpersecond(DEFAULT_SPEED) / self.frame_rate
                    else:
                        my_target_speed = plan_helper.mph_to_meterpersecond(DEFAULT_SPEED) / self.frame_rate
    ...
    def plan_ego(self, current_state, current_frame_idx):
        ...
        if self.is_planning(current_frame_idx) and current_state is not None:
            ...
            my_current_v_per_step = utils.euclidean_distance(
                    ...
                    for j, each_other_traj in enumerate(others_trajectory):
                        ...
                        if dist > 10:
                            for tl_pt in traffic_light_ending_pts:
                                ...
                                if running:
                                    ...
                                    ego_tf_yaw = plan_helper.get_angle_of_a_line(pt1=ego_pose[:2], pt2=tl_pt[:2])
                                    target_tf_yae = plan_helper.get_angle_of_a_line(pt1=target_pose[:2], pt2=tl_pt[:2])
                        ...
                        if current_time < ego_time_length - 1:
                            ...
                            ego_agent2 = Agent(x=(ego_pose2[0] + ego_pose[0]) / 2,
                                               ...
                                               length=max(2, utils.euclidean_distance(ego_pose2[:2], ego_pose[:2])),
                                               width=current_state['agent'][ego_agent_id]['shape'][0][0],
                                               agent_id=ego_agent_id)
                        ...
                        if has_collision:
                            ...
                            if total_frame_in_target - current_time > 5:
                                target_speed = utils.euclidean_distance(each_other_traj[current_time, :2], each_other_traj[current_time + 5, :2]) / 5
                            elif current_time > 5:
                                target_speed = utils.euclidean_distance(each_other_traj[current_time - 5, :2], each_other_traj[current_time, :2]) / 5
                            else:
                                target_speed = 0
            ...
            if len(selected_route) > 0:
                ...
                if current_lane_speed_limit is not None:
                    my_target_speed = plan_helper.mph_to_meterpersecond(current_lane_speed_limit) / self.frame_rate
            ...
            if ego_to_yield:
                ...
                elif my_current_v_per_step < 1 / self.frame_rate and utils.euclidean_distance(my_traj[0, :2], my_traj[-1, :2]) < MINIMAL_DISTANCE_TO_TRAVEL:
                    planed_traj = self.get_trajectory_from_interpolator(my_interpolator=my_interpolator,
                                                                        my_current_speed=my_current_v_per_step,
                                                                        desired_speed=my_target_speed,
                                                                        hold_still=True)
                ...
                if utils.euclidean_distance(my_traj[0, :2], my_traj[-1, :2]) < MINIMAL_DISTANCE_TO_TRAVEL:
                    planed_traj = self.get_trajectory_from_interpolator(my_interpolator=my_interpolator,
                                                                        my_current_speed=my_current_v_per_step,
                                                                        desired_speed=my_target_speed,
                                                                        hold_still=True)
