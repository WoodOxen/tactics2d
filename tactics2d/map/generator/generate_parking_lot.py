...
def _get_rand_pos(origin_x, origin_y, angle_min, angle_max, radius_min, radius_max):
    ...
    radius_rand = _random_gaussian_num((radius_min+radius_max)/2, (radius_max-radius_min)/4, radius_min, radius_max)
...
def _gene_bay_left_obst(dest_pos, vehicle_box):
    ...
    if random()<0.5: # generate simple obstacle
        ...
        left_obst_rf = _get_rand_pos(*car_lf, pi*11/12, pi*13/12, min_dist_to_obst, max_dist_to_obst)
        left_obst_rb = _get_rand_pos(*car_lb, pi*11/12, pi*13/12, min_dist_to_obst, max_dist_to_obst)
        ...
        max_dist_to_obst = 1.0
        min_dist_to_obst = 0.4 + MIN_DIST_TO_OBST
        left_car_x = origin[0] - (WIDTH + _random_uniform_num(min_dist_to_obst, max_dist_to_obst))
        left_car_yaw = _random_gaussian_num(pi/2, pi/36, pi*5/12, pi*7/12)
        rb, _, _, lb  = _get_box_coords([left_car_x, origin[1], left_car_yaw], vehicle_box)
        min_left_car_y = -min(rb[1], lb[1]) + MIN_DIST_TO_OBST
        left_car_y = _random_gaussian_num(min_left_car_y+0.4, 0.2, min_left_car_y, min_left_car_y+0.8)
...
def _gene_bay_right_obst(dest_pos, vehicle_box, obstacle_left):
    ...
    max_dist_to_obst = 1.0
    if random()<0.5: # generate simple obstacle
        right_obst_lf = _get_rand_pos(*car_rf, -pi/12, pi/12, min_dist_to_obst, max_dist_to_obst)
        right_obst_lb = _get_rand_pos(*car_rb, -pi/12, pi/12, min_dist_to_obst, max_dist_to_obst)
        ...
        right_car_x = origin[0] + (WIDTH + _random_uniform_num(min_dist_to_obst, max_dist_to_obst))
        right_car_yaw = _random_gaussian_num(pi/2, pi/36, pi*5/12, pi*7/12)
        rb, _, _, lb  = _get_box_coords([right_car_x, origin[1], right_car_yaw], vehicle_box)
        min_right_car_y = -min(rb[1], lb[1]) + MIN_DIST_TO_OBST
        right_car_y = _random_gaussian_num(min_right_car_y+0.4, 0.2, min_right_car_y, min_right_car_y+0.8)
...
def gene_bay_park(vehicle_box:LinearRing):
    ...
    obstacle_right, generate_success = _gene_bay_right_obst(dest_pos, vehicle_box, obstacle_left)
    # check collision
...
def _gene_parallel_left_obst(dest_pos, vehicle_box):
    ...
    if random()<0.5: # generate simple obstacle
        max_dist_to_obst = 1.0
        min_dist_to_obst = 0.4 + MIN_DIST_TO_OBST
        left_obst_rf = _get_rand_pos(*car_lb, pi*11/12, pi*13/12, min_dist_to_obst, max_dist_to_obst)
        left_obst_rb = _get_rand_pos(*car_rb, pi*11/12, pi*13/12, min_dist_to_obst, max_dist_to_obst)
        obstacle_left = LinearRing((
            left_obst_rf,
            left_obst_rb,
            (origin[0]-bay_half_len, origin[1]),
        ...
        max_dist_to_obst = 1.0
        min_dist_to_obst = 0.4 + MIN_DIST_TO_OBST
        left_car_x = origin[0] - (LENGTH + _random_uniform_num(min_dist_to_obst, max_dist_to_obst))
        left_car_yaw = _random_gaussian_num(0, pi/36, -pi/12, pi/12)
        rb, rf, _, _  = _get_box_coords([left_car_x, origin[1], left_car_yaw], vehicle_box)
...
def _gene_parallel_right_obst(dest_pos, vehicle_box, obstacle_left):
    ...
    max_dist_to_obst = 1.0
    if random()<0.5: # generate simple obstacle
        right_obst_lf = _get_rand_pos(*car_lf, -pi/12, pi/12, min_dist_to_obst, max_dist_to_obst)
        right_obst_lb = _get_rand_pos(*car_rf, -pi/12, pi/12, min_dist_to_obst, max_dist_to_obst)
        obstacle_right = LinearRing((
            (origin[0]+bay_half_len, right_obst_lf[1]),
            (origin[0]+bay_half_len, origin[1]),
            right_obst_lb,
        ...
        right_car_x = origin[0] + (LENGTH + _random_uniform_num(min_dist_to_obst, max_dist_to_obst))
        right_car_yaw = _random_gaussian_num(0, pi/36, -pi/12, pi/12)
        rb, rf, _, _  = _get_box_coords([right_car_x, origin[1], right_car_yaw], vehicle_box)
...
def gene_parallel_park(vehicle_box:LinearRing):
    ...
    # generate obstacle on right
    obstacle_right, generate_success = _gene_parallel_right_obst(dest_pos, vehicle_box, obstacle_left)
    # check collision
    obstacles = [obstacle_back, obstacle_left, obstacle_right]
    for obst in obstacles:
        if obst.intersects(dest_box):
