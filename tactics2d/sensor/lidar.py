...
class Lidar(SensorBase):
    ...
    def __init__(
            ...
            freq_scan: float = 10.0, freq_detect: float = 5000.0
        ):
    ...
    def _scan_obstacles(self, participants, frame: int = None):
        ...
        lidar_lines = [
            LineString([self.position, 
                (self.position.x + self.perception_range * np.cos(i * self.angle_resolution + self.heading),
                self.position.y + self.perception_range * np.sin(i * self.angle_resolution + self.heading))
